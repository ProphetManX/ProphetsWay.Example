...
namespace ProphetsWay.Example.DataAccess.NoDB.Daos
{
	...
	/// </remarks>
	internal class CompanyResourceDao : ICompanyResourceDao
	{
		/// <inheritdoc />
		public void Insert(CompanyResource item)
		{
			...
			lock (DataStore.CompanyResources)
			{
				...
				DataStore.CompanyResources.Add(Copy(item));
			}
		}
		...
		public int Delete(CompanyResource item)
		{
			...
			lock (DataStore.CompanyResources)
			{
				...
				DataStore.CompanyResources.Remove(stored);
				return 1;
			}
		}
		...
		public IList<CompanyResource> GetAll(CompanyResource item)
		{
			...
			lock (DataStore.CompanyResources)
				return DataStore.CompanyResources.Select(Copy).ToList();
		}
		...
		{
			return DataStore.CompanyResources.FirstOrDefault(x => x.CompanyId == companyId && x.ResourceId == resourceId);
		}

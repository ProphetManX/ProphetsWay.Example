...
a re-check of the README. **Amended again 2026-08-16** on branch `3.1.1-eftool-findings`: the suite is
now **164 tests / 328 executions**, the version line reads **3.1.1**, `IExampleDataAccess` carries
**four** DAL-wide rules, and the EFTools submodule pointer has been advanced. Evidence-based; every
claim cites a source file._
...
| Suite | 164 tests × 2 legs = **328 executions** | Static count, 2026-08-16 — see below |
| Version | 3.1.1, set by hand | [app-variables.yml](../app-variables.yml#L3) |
...
and moving an over-claiming one into a new `Characterization` sibling. **The same two files were then
extended with a test each**, closing a gate hole where a cascading `Update` had been passing every
`Contract` test; `UserDaoTests.ShouldCallCustomUserFunctionality` moved to `Characterization` in the
same pass. The suite is therefore **164 tests, 328 executions** today. No green run has been recorded
against that tree; the count below is a static one and is labelled as such.
...
| `Scope` trait partition | Static count of class- and method-level `[Trait("Scope", …)]` against `[Fact]` / `[Theory]` / `[InlineData]` | **139 Contract / 5 Characterization / 20 Dispatcher** — sums to 164, the suite total |
...
Expect 164 tests on each of `net48` and `net10.0` — **328 executed test cases total**. The solution
...
[AssertEveryStampIsUtc](../ProphetsWay.Example.Tests/DepartmentDaoTests.cs#L1001). Both helpers are
still present and still public, verified 2026-08-16. **The count of two is from the `Modernizer`
build and has not been re-measured** — the class carries roughly twenty other `public static`
members, so treat the number as unverified until a build is run. Cosmetic either way; making them
`internal` would silence them.
...
paradigm — a worked domain, one in-memory DAL, and a 164-test suite written against interfaces so
...
[IExampleDataAccess](../ProphetsWay.Example.DataAccess/IExampleDataAccess.cs#L10) carry **four**
DAL-wide rules — a **snapshot rule** (reads return deep copies, writes read rather than adopt their
argument), an **ordering rule** (unspecified but stable order; paged windows partition a full pass),
an **identifier rule** (`Insert` assigns the generated identifier back onto the caller's instance),
and a **row count rule** (`Update` and `Delete` return the number of rows matched and written). The
last two were added on 2026-08-16; earlier editions of this document said there were two, which was
accurate at 3.1.0. Each is written with an explanation of *why an in-memory store satisfies it
incidentally while SQL Server does not*, which is precisely the class of divergence that would
...
retarget. **The working tree has since moved on** — two test files were retraited, split and then
extended, and `IExampleDataAccess.cs` gained two rules, taking the suite from 160 tests to 164 — so
the byte-identical claim describes v3.1.0 as released and must not be restated about the current tree.
...
| `ProphetsWay.Example.Tests` | xUnit | 164 tests written against the interfaces |
...
`net10.0`, two test legs, a `3.1.0` reference, and the EFTools claim qualified. The count rows went
stale a second time when two tests were added on 2026-08-16 and were corrected again in the same pass
that corrected this document.

| Existing claim | Verdict | Evidence |
|---|---|---|
| **"138 Contract / 2 Characterization / 20 Dispatcher"** ([README.md#L137](../README.md#L137)) | **WAS STALE TWICE — now 139 / 5 / 20 of 164** | Accurate when written. The two retraits made it 138 / 4 / 20 of 162; the two added tests and the `ShouldCallCustomUserFunctionality` retrait make it **139 / 5 / 20 of 164**. The README table and its "all but five" list were updated in the same pass |
...
| Test-suite size and legs | **WAS STALE — corrected** | The README now says two legs, `net48` and `net10.0`, and **164 tests / 328 executions** |
...
| **"`ProphetsWay.EFTools` carries an Entity Framework implementation of the very same `IExampleDataAccess` contract, and the tests do not change to accommodate it."** | **Now qualified — accurate as written** | The README states that the pointer was advanced to `d845863` (the 3.1.0 tree) and that the EF implementation is still on `ProphetsWay.BaseDataAccess` 2.5.0, so the demonstration is pending. Verified against `ProphetsWay.EFTools/.git/modules/ProphetsWay.Example/HEAD` and `ProphetsWay.Example.DataAccess.EF.csproj`. The underlying coordination gap is unchanged: [FR 5](feature-requests.md#5--advance-the-eftools-submodule-pointer-onto-the-3x-contracts) |
...
1. **The repo's headline claim is still pending, but the reason has moved.** `ProphetsWay.EFTools`
   pins this repo as a submodule, and **the pointer was advanced on 2026-08-16 to `d845863`** — the
   tip of `main`, i.e. the 3.1.0 tree — so the contracts it carries are current. What has not landed
   is EFTools' own adoption of them: both
   `ProphetsWay.EFTools/ProphetsWay.EFTools/ProphetsWay.EFTools.csproj` and
   `ProphetsWay.EFTools/ProphetsWay.Example.DataAccess.EF/ProphetsWay.Example.DataAccess.EF.csproj`
   still reference `ProphetsWay.BaseDataAccess` **2.5.0**. **This remains a coordination requirement,
   not duplication** — a submodule cannot drift, only lag. Never edit files under
   `ProphetsWay.EFTools/ProphetsWay.Example/`; edit here and advance the pointer.
   [FR 5](feature-requests.md#5--advance-the-eftools-submodule-pointer-onto-the-3x-contracts), whose
   stated status of `Proposed` now covers only the remainder of the work — its first step has landed.
